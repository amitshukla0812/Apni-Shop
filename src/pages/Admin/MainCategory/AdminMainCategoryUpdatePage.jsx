...
export default function AdminMainCategoryUpdatePage() {
    ...
    function getInputData(e) {
        ...
        let value = name === "pic" && e.target.files.length ? "maincategory/" + e.target.files[0].name : e.target.value

        setData({ ...data, [name]: name === "status" ? value === "1" : value })
    ...
    function postData(e) {
        ...
        if (error)
            ...
            if (item) {
                setErrorMessage({ ...error, name: "Maincategory with ths name already exist" })
                setShow(true)
            }
            else {
